...
from app import db
from app.models import Kelas, Mapel, Pengajaran, Siswa, KegiatanIndustri, Ekstrakurikuler, Semester, RekapAbsensi, NilaiAkhir, SiswaKelas, TahunAjaran
...
def upload_nilai_akhir():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            required_columns = ['nama_siswa', 'mapel', 'tahun_ajaran', 'semester', 'nilai', 'capaian_kompetensi']
    ...
    try:
        ...
        for i in range(0, len(df), BATCH_SIZE):
            ...
            for _, row in batch.iterrows():
                ...
                # Get pengajaran
                pengajaran = Pengajaran.query.join(Mapel).filter(
                    Mapel.nama_mapel == row['mapel'],
                    Pengajaran.id_semester == semester.id,
                    Pengajaran.id_kelas == siswa.id_kelas
                ).first()
                if not pengajaran:
                    error_messages.append(f"Data pengajaran untuk mapel '{row['mapel']}' tidak ditemukan.")
                    continue

                # Get siswa_kelas
                siswa_kelas = SiswaKelas.query.filter_by(
                    id_siswa=siswa.id_siswa,
                    id_kelas=siswa.id_kelas
                ).first()
                if not siswa_kelas:
                    error_messages.append(f"Data kelas untuk siswa '{row['nama_siswa']}' tidak ditemukan.")
                    continue
                ...
                if existing_entry:
                    error_messages.append(f"Data untuk siswa '{siswa.nama}' dan mapel '{row['mapel']}' sudah ada.")
                    continue
